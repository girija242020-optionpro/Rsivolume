...
if(VAPID_PUBLIC_KEY&&VAPID_PRIVATE_KEY&&VAPID_SUBJECT){try{webpush.setVapidDetails(VAPID_SUBJECT,VAPID_PUBLIC_KEY,VAPID_PRIVATE_KEY)}catch(e){console.log('VAPID error:',e.message)}}
async function push(payload){if(!VAPID_PUBLIC_KEY||!VAPID_PRIVATE_KEY)return;for(const [id,s] of [...pushes]){try{await webpush.sendNotification(s,JSON.stringify(payload))}catch(e){if(e.statusCode===404||e.statusCode===410)pushes.delete(id);else console.log('Push failed:',e.message)}}}
...
app.post('/api/v1/push/subscribe',(req,res)=>{const {subscription,clientId='default'}=req.body||{};if(!subscription?.endpoint)return res.status(400).json({success:false,error:'subscription required'});const id=crypto.createHash('sha256').update(subscription.endpoint).digest('hex');pushes.set(id,{...subscription,clientId});res.json({success:true,subscribers:pushes.size})});
app.post('/api/v1/push/test',async(req,res)=>{await push({type:'TEST',title:'RSI DEMA Volume',body:'Test alarm pipeline is working.'});res.json({success:true,subscribers:pushes.size})});
app.post('/api/v1/push/event',async(req,res)=>{const b=req.body||{};if(!b.title||!b.body)return res.status(400).json({success:false,error:'title and body required'});await push({type:b.type||'EVENT',title:String(b.title),body:String(b.body)});res.json({success:true,subscribers:pushes.size})});
